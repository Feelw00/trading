...
2. ~~P-2 미분류 LLM 폴백 분류기~~ → **07-11 완료**. ~~P-9 구현 착수~~ → **07-11 1단계 완료**.
   **P-9 2단계**: ① R6 저녁 보고 "스윙 기회" 섹션(SwingStore.triggers_on 소비 → 결재 노출)
   ② R7 트리거 적중률 채점 축(발화 후 N일 수익 → 임계·가중치 튜닝)
   ③ flows 수집 범위를 스윙 유니버스로 확장(수급 커버리지 26/90 해소)
