...
**7/14 D1 관찰 포인트**: ~~R4 조사~~→완료(정당 기각 — EXEC-7로 구조 해결). ① 레짐 채널 첫 실전(전일 기준 6,806.93) ② 테스트 진입 장치(11시, EXEC_TEST_ENTRY — 검증 후 .env에서 제거) ③ 가드 4종(붕괴·소진·레짐·상한) 동작 ④ 사다리·회수 경로 로그. M4 잔여: 서킷브레이커 신호·환율 임계 감시 패스.
