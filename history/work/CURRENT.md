...
## 최근 완료
- 2026-06-10 — **M2 마무리 슬라이스 4종 + R4 실검증(18건 검증, confirmed 0 — 결함 2건 발견: R2 affected 공백, claude -p 실패 집계)** → [archive](archive/2026-06-10-m2-wrapup-slices.md)
...
**M2 잔여** (빠른 슬라이스)
1. **R2 affected 채움 수정** — L1 후보 배치의 종목 미귀속으로 R4 linkage 자동 기각·R3 연결 단절 (6/10 실검증 발견). 수정 후 R4 재실행
2. **claude -p 견고화** — 타임아웃 120s 상향(.env knob) + JSON 파싱 + 1회 재시도(§9 정합). 실검증 54콜 중 8콜 실패가 기각으로 집계됨
3. R4 러너 진행 콜백(BatchProgress 패턴)
4. CAL-1: 2026년 음력·대체공휴일 KRX 공지 확인 → `krx_holidays.json` 채우기
5. R1 일반 게이트 운영 배선 — landing→FactRecord 변환 계층 후 거시·시세 적용
