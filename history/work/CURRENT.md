...
- **관찰 대기(자동 체인, 개입 없이 확인만)**: ① 9/4 18:00 eod-v3 — **교정된 트리거 첫 실행(cron list status `ok` 기대)** ·
  ...
  ③ 이익 보호·사이클 재등록 첫 실발동 · 태깅 스킵 재시도 소진(→ P-19 ④ 박제 시점).
...
1. **COLLECT-5 후속 결재 — 실측 완료(9/4), 결정 가능** → `docs/research/2026-09-04-collect5-reit-dividends-split-method.md` §3.
   ① 리츠: 권고 **(a) 면제 해제 + `alot_facts` 접수분별 저장**(리츠 22종이 반기·분기 결산이라 첫 접수분만 저장되는 현 구조는 연간
   배당 50~75% 과소, 코어 판정 변동 0) ② 분할: 권고 **(a) 인적분할 강등 해제·물적 현행 강등 유지·미상은 보수 유지**, 물적 배제 승격은
   **반대**(통과 33종 탈락, 헌법 표적은 "물적분할 후 자회사 상장" — 상장 사실 소스 확정 후 별도). 소스 확정: `cmpDvDecsn.json` `dv_mth`.
...
3. ~~COLLECT-5 ①② 실측~~ → **완료(9/4)**: 리츠 분배금은 `alotMatter`에 있음(라벨 '보통주식' + 접수분 2~4건 구조) · 분할 구분은
   `cmpDvDecsn.json` `dv_mth`로 가능(59개 기업 전수 분류). 구현은 A-1 결재 후.
