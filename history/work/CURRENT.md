...
- **작업 부팅 + 수집 트리거 구축** — 완료분:
  ...
  - **검증:** mypy 36 clean · pytest 38 passed. **거시 11건 실적재 전부 verified** — FRED(SOX·S&P·NASDAQ·WTI·Brent) / ECOS(USD/KRW 1543·기준금리 2.5%·국고채 3Y 3.882%·10Y 4.254%) / 공공데이터(KOSPI 8160.59·KOSDAQ 1002.44, EOD +1영업일).
...
## 다음 후보
1. **KIS 어댑터/MCP**(국내 종목 시세·호가·수급, 섹터 9클러스터) — 계좌 필요(모의·trading off).
2. **뉴스 2데스크**(거시·정책/시황·해외) — LLM 수집 경로.
3. 클러스터/종목 `domains.py CLUSTERS` 승격 + `/collect` 배선 + openclaw cron(하루 2회·gpt-5.5).
4. 남은 갭 — 국내 수급 KIS TR, NXT(🔴). **M2** R1 게이트(순수코드).
