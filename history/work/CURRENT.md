...
## 진행 중
- **실투자 병행 개시(2026-09-02)** — 토스 실측(9/2 12시): 아이퀘스트 13주(2,720) · 원림 2주
  (15,350) · 신세계I&C 2주(13,090) · 대한약품 1주(26,950) · 에이스침대 1주(33,000). 평단 =
  가이드 시작가. (구 기록 "각 100주"는 정규화 단위와 혼동한 오기 — 정정.)
- **EXEC-12 가이드 매도 예약(9/2)**: `guide-orders` 평일 08:40 — 실보유 × 다음 매도선 조건주문,
  **수량 변경 시에만 취소·재등록**(유지 판정 동일 계획·동일 보유·만료 2일 초과), 모드 **dry-run**
  (5거래일 관찰 후 운영자 live 전환). **시작가 불변**(추가 매수에도 rebase 금지 — 정정 전용).
...
- **자동 체인** — eod-v3 평일 18:00(+밸류에이션·페이퍼 마킹) · weekly-v3 토 09:30
  (지배주주지분→밸류에이션→온도계→스크리닝→환원·분할→자동 심사 rule-v1→페이퍼 자동
  등록→다이제스트). 웹: 포트 80(선정 후보 /picks · 매매 가이드 /paper 신설).
