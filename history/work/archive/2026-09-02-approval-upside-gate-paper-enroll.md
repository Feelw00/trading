...
## 후속 UI 정리(같은 세션, 운영자 확정)
- 괴리 임계 **15% 확정**(운영자 "15%로 확정"). GUIDE-1은 자동 반영 여부·심사 외 실보유 취급만 대기.
- /paper 괴리 표시: A안(추정 목표가 셀에 % 병기) → 운영자 "빡빡하다" → **추정 목표가 열 제거, 목표가
  셀 호버 툴팁**("오늘 추정 목표가 43,286 · 등록 대비 -3%"), ±15%는 ⚠ 배지만. 표 1 매수 상한·매도선·
  다음 매도선, 표 2 예약 매도·수량의 `hl` 색 강조 제거(표 1 = 9열).
- 마지막 행 툴팁 잘림: `.scroll{overflow-x:auto}`가 세로도 클리핑 → `layout.py`에
  `.scroll:has(.tip:hover){overflow:visible}`(호버 중에만 해제, JS 없음 유지). 운영자 확인.
- 커밋 4건: 8c3a196(feat) · 8e372b0(docs) · f89cd4d(A안) · 3da1922(표 정리·클리핑).
