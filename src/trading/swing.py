...
class SwingConfig:
    ...
    mdd_window: int = 120
    # 최대낙폭 컷(유니버스 제외). -0.40 → -0.55 (운영자 결정 2026-07-12): -40%는 2026 고변동
    # 강세장에서 분포 중앙값에 걸려 127/238 탈락 + 추세·재무 최상위(제주반도체·에스에이엠티 등)
    # 원천 배제. -55%는 하위 ~28%만 컷 — 변동성 페널티는 추세 축(변동성 조정 모멘텀)이 담당.
    mdd_cut: float = -0.55
    ma_short: int = 20
    ma_long: int = 60
    # ── 도메인 열기 ──
