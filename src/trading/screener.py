...
# 병합 우선순위: KRX 공식 업종(운영자 결정 2026-07-13 — 거래소 분류 최우선) >
# 수기 큐레이션 > 멀티에이전트 LLM > grounded > LLM 폴백(잔존 갭만).
# 앞 소스가 종목별 우선 — 하위 소스는 상위 소스를 절대 덮지 않는다(KIS 결측 종목만 채움).
SECTOR_SOURCES = ("kis-bstp-v1", "manual-curated-v1", "llm-cls-v1", "dart-ksic-v1", "llm-fallback-v1")
