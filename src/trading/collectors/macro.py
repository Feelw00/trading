...
MACRO_ITEMS: tuple[MacroItem, ...] = (
    ...
    # 국내지수 — 공공데이터 금융위 지수시세(EOD, idxNm 정확값)
    MacroItem("KOSPI", "index_level", "index", "DATAGOKR", region="KR", unit="pt", idx_nm="코스피"),
    MacroItem("KOSDAQ", "index_level", "index", "DATAGOKR", region="KR", unit="pt", idx_nm="코스닥"),
...
def collect_macro(
    ...
    datagokr: DataGoKrIndexClient | None = None,
    *,
    ...
    for it in items:
        try:
            if it.source == "FRED":
                ...
                summary.verified += 1
            elif it.source == "DATAGOKR":
                if it.idx_nm is None:
                    summary.blocked.append(f"{it.name}: 지수명 미설정")
                    continue
                if datagokr is None:
                    summary.blocked.append(f"{it.name}: DATA_GO_KR_API_KEY 미설정")
                    continue
                idx_res = datagokr.latest(it.idx_nm, start, end)
                if idx_res is None:
                    summary.blocked.append(f"{it.name}: data.go.kr 데이터 없음")
                    continue
                as_of, value = idx_res
                summary.facts.append(
                    CollectedFact(
...
def main() -> int:
    ...
    datagokr_key = os.environ.get("DATA_GO_KR_API_KEY", "")
    fred = FredClient(fred_key) if fred_key else None
    ecos = EcosClient(ecos_key) if ecos_key else None
    datagokr = DataGoKrIndexClient(datagokr_key) if datagokr_key else None

    summary = collect_macro(fred, ecos, datagokr)
    if summary.facts:
