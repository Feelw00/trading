...
3. `poetry run python -m trading.screener` — 갱신된 DB로 후보 + 섹터 태그 산출.
4. 보고: 수집 일자·신규 행수 / 스크리너 후보 Top-N(섹터 태그) / blocked.

> **하네스(COLLECT-3):** 승인 소스(data.go.kr) 어댑터만 호출. **독자 웹서치 금지.** 키 미설정·소스 실패는 `blocked` 보고 — 다른 소스·웹서치로 대체하지 않는다.
