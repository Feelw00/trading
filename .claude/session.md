...
- **9/3 결재(policy v2.15·GUIDE-1·EXEC-14)**: 과열 산업(⚠)은 `paper register` 불가(승인 노출은 유지) · 심사 승인
  ...
  DCA **영구 누락** — 9/7 EXEC-14, 자금 유입 불규칙) · 매도 예약은 다음 선 1개만(의도) · 전량 정리는 앱 직접 — 세션이 매수·정리 주문을 내지 않는다
...
- 큰 줄기: v0.3 §10 로드맵 — Phase 0(환경 복구·브로커 대사) → 1(밸류에이션·백필) → 2(온도계·화이트리스트 결재) → 3(스크리너·페이퍼) → 4(분기 R7·청산 경로 — DCA 집행은 9/7 영구 누락)
