...
### 🟡 CAL-3 — KRX 거래시간 연장(프리·애프터마켓) 2026-09-14 시행 — 세션 가드·프리마켓 데이터 영향
- **맥락:** KRX가 프리마켓·애프터마켓을 도입하는 거래시간 연장을 **당초 2026-06-29 → 2026-09-14로 연기**([노컷뉴스](https://www.nocutnews.co.kr/news/6486442), [전자신문](https://www.etnews.com/20260317000256)). 프리마켓 마감은 07:50(대체거래소 08:00 개장과 10분 버퍼).
- **영향(미확인):** ① `market_calendar`의 세션 정의(`KRX_OPEN 09:00`/`KRX_CLOSE 15:30`)가 정규장 기준으로 유지되는지 ② 장중 LLM 휴면 범위(설계서 §5)를 프리·애프터까지 넓힐지 ③ **SEL-1(프리마켓 흐름변수)이 NXT 의존 없이 KRX 자체 프리마켓으로 해소될 수 있는지**.
- **잠정 처리:** 코드 변경 없음(현행 정규장 가드 유지 — 시행 전이라 무해). **시행 전(9월 초) KRX 최종 공지로 정규장 시간·데이터 공개 범위 확인 후 결정.**
