...
### 🟡 CAL-2 — 장중 LLM 가드의 적용 범위 (cron 한정 vs 수동 실행 포함)
- **맥락:** 설계서 §5 "장중(09:00–15:30) LLM 라운드 전면 휴면". 가드(`require_llm_rounds_allowed`)는 구현됨. 이를 `trading.run` 디스패치에 항상 강제하면 **운영자 수동 실행**(예: 장중 `/collect` 후 score-news 백필)도 차단된다.
- **잠정 처리:** 가드는 라이브러리로 제공만 하고 디스패치 배선은 **cron 활성화 작업(M3)에서 결정** — cron 경유 실행에는 강제, 수동 실행 허용 여부는 운영자 결정 대기. 그 전까지 `trading.run`은 가드 미배선(현행 유지).
