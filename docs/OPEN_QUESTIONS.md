...
### 🟡 CAL-1 — KRX 휴장일 데이터 출처 (음력·대체공휴일·임시휴장)
- **맥락:** `market_calendar` 가드(SCHED-1)가 거래일을 판정한다. 월-일 고정 공휴일(신정·삼일절·근로자의날·어린이날·현충일·광복절·개천절·한글날·성탄절·연말휴장 12/31)은 코드에 확정. **음력 공휴일(설날·추석·석가탄신일)·대체공휴일·임시휴장은 추측 금지** — 연도별 명시 날짜가 필요.
- **잠정 처리:** `src/trading/market_calendar/krx_holidays.json`(운영자 유지, KRX 공지 확인분만)으로 주입. 미등록 휴장일의 실패 방향은 안전(가드가 LLM을 더 막을 뿐). 12/31이 주말인 해의 직전영업일 휴장 여부도 KRX 공지로 확인해 파일에 반영.
- **후보 해소책:** data.go.kr 공휴일 API 또는 KRX 휴장일 안내 페이지 어댑터(승인 소스 확정 후 구현).
- **2026-07-11 실측(연속성 가드):** 시세 연속성 점검(`python -m trading.collectors.market --check`)이 달력 미등록 휴장일 **9일**을 드러냈다. 승인 소스(data.go.kr)가 **정상 응답 + 무자료**로 답한 날들이다(소스 장애는 `CollectError` raise이므로 빈 응답=휴장 관측):
  `20251006, 20251007, 20251008`(추석 연휴 추정) · `20260216, 20260217, 20260218`(설 연휴) · `20260302`(삼일절 대체) · `20260525`(석탄일 대체) · `20260603`(지방선거).
  **달력 파일은 추측으로 고치지 않았다** — 관측은 `data/market.sqlite`의 `no_data_days`(append-only)에 박제해 재경고만 막았다. **운영자가 KRX 공지로 확인해 `krx_holidays.json`에 반영하면 CAL-1 종결.**
