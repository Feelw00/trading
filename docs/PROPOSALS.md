...
결재 요청 항목:
- **A(순도)**: ① 건설기계 큐레이션 전환(두산밥캣·HD현대인프라코어 등 KSIC 실측 후 구성안)
  ② 철강 큐레이션 전환 또는 LIG넥스원 제외 방식 결정 ③ 해운 큐레이션 전환(HMM·팬오션 등).
- **B(실증 트랙)**: 계측 전용 큐레이션 신설 — 방산(한화에어로·LIG·KAI·현대로템·한화시스템),
  2차전지(LG엔솔·삼성SDI 등). R4 미편입, 왕복 실증 축적용.
- **C(편입 규칙)**: 비WL 그룹이 [완결 왕복 ≥1 + 진폭 ≥2배 + 구조적 사양 아님 + 하단 존
  진입]이면 분기 R7 안건 자동 상정 — 편입은 항상 운영자 결재.
