...
## 🆕 P-19 — EXEC-12 · /picks 후속 후보 (세션 발견 2026-09-02 · ① 기각 2026-09-03, 나머지 미결재)
- ~~① 가이드 매도 예약: 다음 선 1개 → **남은 전 선 동시 등록**(하루에 여러 선을 뚫는 급등 대응).
  토스가 같은 종목 복수 SINGLE 조건주문의 수량 합을 보유로 허용하는지 실측 필요.~~ → **❌ 기각(운영자 2026-09-03:
  "다음 매도선 1개만 거는 게 의도")** — 실주문 표면 최소화. 급등일 윗선의 하루 지연은 수용(policy v2.15 ⑤).
- ② 실체결 ↔ 페이퍼 원장 정합 — 가이드 수익률은 정규화 단위 유지하되, 실체결 이벤트를
  가이드 페이지에 표기(실현 이력).
- ③ /picks 대기 큐 산업 비례 캡(예: 산업 코어 수의 제곱근) · 큐 체류 N주 초과 시 후순위 —
  순환 1~2주 관찰 후 결정.
- ~~④ KRX 업종 태깅 영구 스킵 129종목(KONEX·신형 코드·초소형) — `source="none"` 박제로 일 129콜 절감.~~
  → **✅ 구현·박제(운영자 2026-09-04 "진행해")**: 재시도 소진 확인(8/31·9/1·9/2 cron 3회 129/129 동일 실패) + 실호출
  5종(KONEX·950 외국기업·신형 코드·NH농우바이오·맵스리얼티) 모두 **정상 응답(79~80필드)인데 `bstp_kor_isnm`=null/공백** —
  일시 장애가 아니라 소스에 업종이 없는 것(실체: KONEX 108 · 외국기업 950번대 10 · 신형 코드 3 · 정규 코스피/코스닥 10, "초소형"
  아님 — 코오롱티슈진 1.5조·맵스리얼티 6,700억 포함). 규칙: `classify_krx`가 **정상 응답(`stck_prpr` 있음)+업종 없음**이면
  `source="none"` 'unclassified' 행 박제(`KRX_NONE_SOURCE`), 호출 실패·빈 응답은 종전대로 행 없이 재시도. 신규 상장분에도 같은 규칙
  자동 적용. 박제분 재시도 = `python -m trading.sectors --retry-pinned`(성공 시 `kis-bstp-v1` 행이 first-wins). 9/4 15:57 수동 실행으로
  129 박제, 재실행 KIS 콜 0 확인. 소비자(`sector_map(KRX_SOURCE)`·산업 상대 위치)는 무영향 — 129종은 전에도 미태깅.
- ⑤ 9/1 신규 통과 17종이 금융(보험·증권)에 몰림 — COLLECT-6 지배주주 PBR 전환 효과인지 추적.
- ~~⑥ (④ 후속, 세션 발견 2026-09-04) weekly-v3에 `sectors --retry-pinned` 주 1회(≈129콜/주)를 넣어 신형 코드·신규 편입 종목의
  업종 배정을 자동 포착할지 — 현재 수동. 코스트 미미, 결재보다는 운영 편의 판단.~~ → **✅ 구현(운영자 위임 2026-09-07 "네 권고대로",
  policy v2.21 ②)**: weekly-v3 첫 단계 `sectors-retry`(best-effort, 실패해도 계측 진행). 수동 명령은 유지. 첫 cron = 9/12(토).


## 🆕 P-20 — 회귀 여력 v2.13/v2.14 후속 후보 (세션 발견 2026-09-03 · ① 채택·⑤ 기각 2026-09-03 · ④⑥⑧ 완료 2026-09-04, 나머지 미결재)
근거·실측: `docs/research/2026-09-03-target-price-industry-practice.md`, `history/work/archive/2026-09-03-own-history-pbr-band.md`.
- ① **과열 산업 제외의 규칙화** — 운영자가 9/3 가이드에서 과열 12종을 수동 제거. 현 P-18은 과열을 ⚠ 플래그로만 둠.
  (a) 가이드 등록 자격에만 "과열 산업 제외"(권고 — 승인 노출은 유지) vs (b) 승인 노출·자동 심사 hold 조건까지 확장.
  규칙화 없으면 다음 `paper register`가 과열 종목을 다시 편입한다.
  → **🟢 (a) 채택(운영자 2026-09-03) — policy v2.15 ① 구현**: `paper.register_block_reason`(승인 → 과열 → 여력 순 순수 함수),
  `paper register`만 거부. 승인 노출·자동 심사·실보유 자동 편입은 불변. 테스트 `test_register_block_reason_gate`.
- ② **목표가 도달률 계측(R7)** — 페이퍼 체결(매도선 터치 일자)이 원료. 사이클이 닫히면 도달 여부·소요일·최대 접근률을
  박제하고 업계 벤치마크(미국 12개월 도달 38%·한국 19%)와 비교 — v2.12→v2.14 산식 변경 효과의 유일한 검증 수단.
- ③ **정당 PBR 파라미터 재캘리브레이션** — COE 10%·g 1%는 완화 쪽(국내 금융 리서치 COE 12~15%). 민감도 큼
  (원림 COE9/g0 +31% ~ COE12/g2 −44%). ②의 데이터가 쌓인 뒤 결재.
- ~~④ **밴드 분모 지배주주지분 승격** — 과거 연간 지배기업 소유주지분 10년 백필 후 현재점·과거점 동시 승격(OPEN_QUESTIONS
  COLLECT-6 후속). 비12월 결산 종목의 4/1 적용 규칙도 결산월 수집 시 정밀화.~~ → **✅ 구현(2026-09-04, v2.19)**: 연간 전체
  재무제표 1콜 백필(지배주주지분·귀속 순이익·접수일, 통과 종목 우선·예산형·weekly 갭 채움) · 밴드 분모 전연도 승격(부분
  결측은 자본총계 유지) · PER 분모 귀속 순이익 · **적용일 = 사업보고서 접수일 다음날**(결산월 대신 — 전체 재무제표 응답에
  결산일 필드 없음, 접수번호 앞 8자리가 정확한 공개 시점). 10년 중 최근 7개년부터(밴드 5년 창 + as-of 시차), 잔여 연도는
  weekly 갭 채움.
- ~~⑤ **EXEC-13 전량 지정가 정리 명령** — "지금 전량 매도" 경로 부재(9/3 정리 3건은 운영자 앱 직접). 토스 어댑터
  `place_limit_order`(v0.2 검증)로 현재 매수호가 지정가 SELL. 실주문 경로 신설이라 OPEN_QUESTIONS 등록·결재 선행.~~
  → **❌ 기각(운영자 2026-09-03: "내가 직접")** — 시스템 허용 주문은 EXEC-12 조건부 매도 예약뿐(policy v2.15 ⑥).
- ~~⑥ **배당수익률 필드 오염 교정** — DART 액면 배당률이 시가 수익률 자리에 들어오는 케이스(흥국 56%, COLLECT-5 ③).
  yield_pct > 20% 전수 스캔 후 파서 행 선택 교정.~~ → **✅ 완료(2026-09-04, policy v2.17 ②)**: 전수 스캔 결과 오염은 "행 선택"이
  아니라 **같은 행에 액면 배당률을 적는 공시**(흥국 5개년 40~56% = DPS÷액면 500원 정확 일치, 12종 23행; FY말 종가 교차로 22행
  확정). 가드 `is_par_based_yield`(수익률 = DPS÷액면가 → None) + 라벨 정규화('보통주식'·공백). 판정 미개입, /picks 표시만 교정
  (>15% 잔존 0). 시가 기준 재계산(DPS÷FY말 종가) 표기는 도입하지 않음 — 기준 변경은 별도 판단.
- ⑦ 불확실성 연동 안전마진(모닝스타식 20~75%) — 보류(위험조정수익률·코어 안정 축이 이미 반영, ② 이후 판단).
- ⑧ **🆕 정당 PBR 캡의 ROE 기준 정합(결재 대기, 2026-09-04 ④ 구현 중 발견)** — ④로 밴드 PBR이 지배주주지분 기준이 됐는데
  v2.14 캡의 ROE₅는 **연결 순이익/연결 자본총계**라 기준이 섞인다. 원장·큐 169종 실측(같은 시세): 접수일 적용 효과 ≈ 0(≥5%p 1종),
  분모 승격으로 ≥5%p 변동 35종 — 대부분 캡 결속 증가(57→65종)에서 발생(밴드 비율 자체는 분모를 통째로 바꾸면 거의 불변).
  예: 원림 +13%→−10%(ROE 3.3% 캡 0.26배가 지배주주 PBR을 누름), 아이퀘스트 +57→+38, 동성케미컬 +51→+41.
  **(b) 승격 종목은 캡의 ROE도 지배주주 기준**(귀속 순이익 ÷ 지배주주지분, 5년 중앙 — ④ 백필로 산출 가능)으로 맞추면 정당 PBR 공식의
  전제(같은 자본에 대한 ROE와 PBR)가 복원됨. 효과: 30% 경계 교차 4종 — 대웅 8→47·헥토이노베이션 26→44(둘 다 hold), KCTC 42→−100,
  **성우하이텍(승인) 44→23 → 승인 보류 파생**(연결 ROE 7.3% vs 지배 4.0% — 비지배 몫 이익). 세션 권고 **(b)** — 현행 혼합 상태는
  어느 순수 기준보다 나쁘다. 미채택 시 대안 (c) 밴드 승격 보류(자본총계 유지, PER·접수일만 적용).
  → **🟢 (b) 채택·구현(운영자 2026-09-04 "권고대로 (b) 가고 구현해", policy v2.20)**: `band.effective_roe`(승격 밴드 = 귀속 순이익÷
  지배주주지분 5y 중앙, `PbrBand.roe_owner_median`) · 귀속 결측은 비지배 ≤0.1%일 때만 연결 순이익 사용(실관측 5종은 비지배 0) ·
  /picks ROE 표시에 기준 병기. 결과: 캡 ROE 지배주주 122/170, 승인 보류 파생 성우하이텍 1종.
- ⑧ PER 밴드 교차 목표 — **반대**(사이클 저점 종목을 체계적으로 배제; 정상화 EPS 변형은 ROE₅ 기반 정당 PBR 캡이 흡수).
