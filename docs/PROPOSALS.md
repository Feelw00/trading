...
> **상태:** 1단계 구현 완료(2026-07-11) — `swing.py`(4축 유니버스+기회 트리거, 순수 코드) +
> `collectors/fins.py`(DART 재무 캐시, 240/243 확보) + `daily-eod` 체인 배선 + `SwingStore` 이력.
> 첫 실행: 게이트 243→유니버스 30, 트리거 11종. 뉴스 stale 가드로 6/15~7/10 공백 오독 차단.
> **2단계 잔여:** R6 저녁 보고 "스윙 기회" 섹션 · R7 트리거 적중률 채점(임계 튜닝) ·
> 수급 커버리지 확대(26/90) · MDD 컷 적정성 검토. 상세: `history/work/archive/2026-07-11-p9-swing-screener.md`.
