...
**ALERT-1 (2026-09-02, 운영자 결정):** 체인 2잡은 끝나면 성공/실패 1통을 텔레그램으로 보내고
(Python 말미, 적체 P1 동봉), 감시 2잡(check-*)이 "오늘 실행 기록 없음"(트리거 턴 실패)을 잡는다.
...
JOBS: tuple[CronJob, ...] = (
    ...
    # --- ALERT-1 미발화 감시 (운영자 결정 2026-09-02) ---
    # 체인 시작 30분 뒤 RunStore(data/runs.sqlite)에 오늘 기록이 없으면 ❌ 미발화, 시작만 있으면
    # ⏳ 미완료를 텔레그램으로 즉시 발송. 완료 기록이 있으면 침묵(실행 보고가 이미 나감).
    CronJob("eod-v3-check", "30 18 * * 1-5", "check-eod-v3",
